...
class JwtRequest:
    ...
    def rpc_response(self, data={}, status=200):
        ...
        if not self.is_ok_response(status):
            return {**r, **data}
        # return {**r, 'data': data}
        # data['success'] = True if self.is_ok_response(status) else False
        # data['code'] = status
        return data
    ...
    def login(self, login, password, with_token=True):
        ...
        try:
            ...
            if not uid:
                return False
            if with_token:
                return {'status': 200, 'token': self.create_token(request.env.user), 'message': 'Login success'}
        except Exception as e:
            print(e)
            if hasattr(e, 'name'):
                if e.name == 'Access Denied':
                    return {'status': 400, 'message': _('The username or password is incorrect')}
                else:
                    return {'status': 500, 'message': _('Server error')}
            else:
                return {'status': 500, 'message': _('Server error')}
        return True
    ...
    def verify(self, token):
        ...
        if len(record) != 1 or record.is_expired:
            return False
        request.env.user = record.user_id
    ...
    def validate_token(self, token, auth=False):
        ...
        # ignore invalid by check user/password once
        """
