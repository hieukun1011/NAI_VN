...
def jwt_auth(req: JwtRequest, *k, **kw):
    try:
        _logger.info(req.token)
        req.validate_token(token=req.token, auth=True)
    except jwt.ExpiredSignatureError:
        raise MiddlewareException('Token was expired', 100)
...
def api_key_middleware(req: JwtRequest, data: MiddlewareData, *k, **kw):
    ...
    api_key = req.headers.get('X-Sogo-Access-Token')
    # print("api_key: " + api_key)
    if api_key != '8bb36621fab8f0ff68660ee5db9758a2':
